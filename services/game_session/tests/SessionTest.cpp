...
class SessionMock : public Session {
    public:
        ...
        auto getNextFrame() -> sharedConstMat_t {
            ...
            if (!_video.set(cv::CAP_PROP_POS_MSEC, GameSession::TICK_RATE.count() * ++_tickCount)) {
                LOG_ERROR(Logger::Quill::getLogger(), "Failed to set video position");
            }

            LOG_DEBUG(Logger::Quill::getLogger(), "Frame number {}, at {}ms", _tickCount, _tickCount * GameSession::TICK_RATE.count());
